...
test('retrieves denied urls for process crawl', async t => {
  ...
  const processCrawl = await client.app.createCrawlerProcessCrawl({
    engine_name: engineName,
    body: {
      dry_run: true
    }
  ...
  const response = await client.app.getCrawlerProcessCrawlDeniedUrls({
    engine_name: engineName,
    process_crawl_id: processCrawl.id!
  })
  t.type(response.total_url_count, 'number')
  t.type(response.denied_url_count, 'number')
  t.type(response.sample_size, 'number')
  t.type(response.denied_urls_sample, 'object')
