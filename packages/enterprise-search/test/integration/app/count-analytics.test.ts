...
test('getCountAnalytics', async t => {
  ...
  const response = await client.app.getCountAnalytics({ engine_name: engineName })
  t.ok(response.results.length >= 0)

  await client.close()
})

test('getCountAnalytics with filters', async t => {
  const client = new Client({
    url,
    auth: { username, password }
  })
  ...
  const response = await client.app.getCountAnalytics({
