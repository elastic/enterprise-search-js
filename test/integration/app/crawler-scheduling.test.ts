...
test('sets a crawl schedule', async t => {
  const client = new Client({
    url,
    auth: { username, password }
  })

  const response = await client.app.putCrawlerCrawlSchedule({
    engine_name: engineName,
    body: {
      frequency: 1,
      unit: 'day'
...
test('gets the crawl schedule', async t => {
  const client = new Client({
    url,
    auth: { username, password }
  })

  await client.app.putCrawlerCrawlSchedule({
    engine_name: engineName,
    body: {
      frequency: 1,
      unit: 'day'
  ...
  const response = await client.app.getCrawlerCrawlSchedule({
    engine_name: engineName
  })
  t.same(response, {
    engine: engineName,
...
test('deletes a crawler crawl schedule', async t => {
  const client = new Client({
    url,
    auth: { username, password }
  })
  ...
  const response = await client.app.deleteCrawlerCrawlSchedule({
    engine_name: engineName
  })
